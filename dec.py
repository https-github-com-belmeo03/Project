...
while count == 1:
 ...
 NoCut_Ver1 = Egde_cut1[0:Height9, NoCode_Begin1-3:NoCode_End1+3]  # ตัดพื้นที่ว่างทั้ง บน และ ล่าง ออก
 ...
 NoCut_Ver2 = NoCut_Ver1[NoCode_Begin-3:NoCode_End+3, 0:Width10]  # ตัดพื้นที่ว่างทั้ง บน และ ล่าง ออก

 HeightBlack2, WidthBlack2 = NoCut_Ver2.shape

# print("H : " + str(HeightBlack2) + "  W : " + str(WidthBlack2))

 for i in range(0, HeightBlack2, +1):
     for j in range(0, WidthBlack2, +1):
         if NoCut_Ver2[i][j] == 255:
             NoCut_Ver2[i][j] = 0
         else:
             NoCut_Ver2[i][j] = 255
     # print(Img_First[i][j])
 arrayList.append(NoCut_Ver2)
 #cv2.imshow("sad", NoCut_Ver2)
 #cv2.imwrite("code"+str(Count_NumWrite)+".png",NoCut_Ver2)
 #Count_NumWrite = Count_NumWrite + 1
 #cv2.waitKey(0)
...
getNumberRasp = []
for i in range (0,arrayList.__len__()-1,+1):
    h,w = arrayList[i].shape
    ...
    test_img3 = cv2.resize(imarrayNew, (20, 20))
